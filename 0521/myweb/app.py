...
def google_authorize():
    try:
        ...
        print(f"2. User Info from Google: {user_info}")
        # user_info 딕셔너리에서 주요 정보 확인 (sub, email, name, picture 등)
        ...
        print(f"3. User lookup by google_id '{google_id}': {'Found' if user else 'Not Found'}")

        if not user:
            # 새로운 Google 사용자라면 데이터베이스에 저장
            email = user_info.get('email')
            # username은 중복되지 않도록 Google email 사용 또는 고유한 값 생성
            # Google 이메일이 없는 경우 대비 (매우 드뭄)
            ...
            print(f"4. New user path: Proposed username: '{username_candidate}'")
            print(f"   - Existing user by proposed username: {'Found' if existing_user_by_username else 'Not Found'}")
            print(f"   - Existing user by Google email: {'Found' if existing_user_by_email else 'Not Found'}")

            # username 충돌 방지 로직 강화 (username과 email 모두 unique이므로)
            if existing_user_by_username or existing_user_by_email:
                # 이메일이나 제안된 유저네임이 이미 존재하면, 구글 ID 기반으로 유저네임 생성
                username = f"google_{google_id}"
                print(f"   - Conflict detected, modified username to: '{username}'")
                # email도 기존에 있다면 None으로 설정하여 unique 제약 위반 방지
                if existing_user_by_email and existing_user_by_email.google_id != google_id:
                     email = None # 기존 이메일이 이미 사용 중인데 다른 Google 계정이라면 email 저장 안함
            else:
            ...
            db.session.add(user)
            db.session.commit()
            print(f"5. New user created in DB: {user}")

        login_user(user) # Flask-Login으로 로그인 처리
        print(f"6. User '{user.username}' (ID: {user.id}) logged in successfully via Google.")
        print(f"--- Google OAuth Callback Debug End ---\n")
        return redirect(url_for('index'))

    except Exception as e:
        # Authlib 관련 에러를 더 자세히 출력하기 위해 예외 처리 강화
        from authlib.integrations.base_client import OAuthError
        if isinstance(e, OAuthError):
            error_message = f"Authlib OAuth Error: {e}"
        else:
            error_message = f"General OAuth callback error: {e}"

        print(f"\n!!! ERROR during Google OAuth callback: {error_message} !!!\n")
        # 실패 시에도 사용자에게 안내 메시지 표시
        return render_template('login.html', error_message=f"Google 로그인에 실패했습니다: {error_message}"), 400
